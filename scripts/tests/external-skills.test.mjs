...
test('repository source registry exists after implementation', async () => {
  ...
  assert.ok(sources.every((item) => item.license.verified && item.license.redistributionAllowed));
  ...
  assert.ok(sources.every((item) => /^[0-9a-f]{40}$/.test(item.version.currentCommit)));
});
