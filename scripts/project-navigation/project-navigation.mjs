...
export async function scanNavigation(root = ROOT) {
  ...
  for (const file of await walk(root, '.agents/skills', (name) => name === 'SKILL.md')) {
    ...
    capabilities.push({ id: `skill-${meta.name}`, kind: 'skill', title: meta.name, domain: meta.name.startsWith('liluo-') ? 'project' : 'general', status: 'available', maturity: 'standard', summary: meta.description ?? '', userCanSay: [], userMustProvide: [], aiCanDo: [], aiMustNotDo: [], skills: [meta.name], agents: [], commands: [], outputs: [], sourceRefs: [file], userEffort: 'low', lastVerified: null, evalStatus: 'unknown' })
  }
