...
export function buildOutlineRelationGraph(input = {}) {
  ...
  for (const card of ragCards) {
    ...
    ensureNode({
      ...
      meta: {
        ...
        expression: {
          ...
          relatedStyleRagRefs: uniqueStrings(shaped.expression?.relatedStyleRagRefs),
    ...
    // Expression → related Style-RAG（挂在主 RAG 节点上，不另建知识/表达子节点）
    for (const styleRef of uniqueStrings(shaped.expression?.relatedStyleRagRefs)) {
      const styleNode = nodes.has(styleRef)
        ? styleRef
        : nodes.has(nodeId('style_rag', styleRef))
          ? nodeId('style_rag', styleRef)
          : null;
      ...
      addEdge({
        ...
        sourceRef: 'expression.relatedStyleRagRefs',
        confidence: 0.7,
        auditStatus: 'auto_generated',
        autoGenerated: true
      });
