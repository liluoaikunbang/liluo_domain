...
export function buildOutlineRelationGraph(input = {}) {
  ...
  const evidenceExcerpts = asArray(input.evidenceExcerpts);
  const evidenceReviews = asArray(input.evidenceReviews);
  const sourceCatalog = asArray(input.sourceCatalog);
  ...
  const evidenceById = new Map(evidenceExcerpts.map((item) => [item.id, item]));
  const sourceById = new Map(sourceCatalog.map((item) => [item.sourceId ?? item.id, item]));
  ...
  // --- Ordinary / professional restraint RAG cards ---
  ...
  for (const card of ragCards) {
    ...
    const evidenceRefs = uniqueStrings([
      ...asArray(shaped.evidenceRefs),
      ...asArray(shaped.knowledge?.evidenceRefs),
      ...asArray(shaped.expression?.evidenceRefs),
      ...asArray(shaped.sourceRefs).map((ref) => ref.evidenceId).filter(Boolean)
    ]);
    const claims = asArray(shaped.knowledge?.factualClaims?.length ? shaped.knowledge.factualClaims : shaped.claims);
    const knowledgeCompleteness = computeBranchCompleteness(shaped.knowledge, 'knowledge');
    const expressionCompleteness = computeBranchCompleteness(shaped.expression, 'expression');
    const overallStatus = deriveOverallStatus(shaped);
    ensureNode({
      id: ragNodeId,
      type: 'rag',
      title: shaped.title || cardId,
      summary: clipSummary(shaped.knowledge?.definition || shaped.definition || shaped.summary || '', 48),
      description: buildRagCardDescription(shaped),
      aliases: uniqueStrings([...(shaped.aliases ?? []), ...(shaped.knowledge?.aliases ?? [])]),
      sourceIds: uniqueStrings([
        cardId,
        ...asArray(shaped.sourceRecordIds),
        ...asArray(shaped.sourceRefs).map((ref) => ref.sourceId || ref.segmentId),
        ...evidenceRefs
      ]),
      conceptIds: uniqueStrings([...(shaped.concepts ?? []), ...(shaped.linkedConceptIds ?? [])]),
      origin: 'external-knowledge-card',
      auditStatus:
        overallStatus === 'conflicted'
          ? 'conflict'
          : shaped.reviewStatus === 'approved' ||
              shaped.reviewStatus === 'reviewed' ||
              shaped.reviewStatus === 'confirmed' ||
              overallStatus === 'confirmed'
            ? 'confirmed'
            : 'pending_review',
      confidence: shaped.canonical ? 1 : 0.7,
      meta: {
        cardType: shaped.cardType,
        ragDomain: shaped.ragDomain,
        ragDomainLabel: shaped.ragDomain === 'general-craft' ? '通用 RAG' : shaped.ragDomain === 'canon' ? '正史 RAG' : '紧缚专业 RAG',
        knowledgeScope: shaped.knowledgeScope,
        reviewStatus: shaped.reviewStatus,
        evidenceStatus: shaped.evidenceStatus,
        contentStatus: shaped.contentStatus,
        retrievalPolicy: shaped.retrievalPolicy ?? null,
        professionalRagVersion: shaped.professionalRagVersion ?? null,
        overallStatus,
        overallStatusLabel: OVERALL_STATUS_LABELS[overallStatus] || overallStatus,
        knowledgeCompleteness,
        expressionCompleteness,
        knowledge: {
          status: shaped.knowledge?.status,
          reviewStatus: shaped.knowledge?.reviewStatus,
          evidenceStatus: shaped.knowledge?.evidenceStatus,
          definition: shaped.knowledge?.definition || '',
          boundaries: shaped.knowledge?.boundaries || { includes: [], excludes: [] },
          distinctions: asArray(shaped.knowledge?.distinctions),
          projectInterpretation: shaped.knowledge?.projectInterpretation || '',
          commonMisreadings: asArray(shaped.knowledge?.commonMisreadings),
          evidenceRefs: uniqueStrings(shaped.knowledge?.evidenceRefs)
        },
        expression: {
          status: shaped.expression?.status,
          reviewStatus: shaped.expression?.reviewStatus,
          evidenceStatus: shaped.expression?.evidenceStatus,
          visualFocus: asArray(shaped.expression?.visualFocus),
          actionLogic: asArray(shaped.expression?.actionLogic),
          movementEffects: asArray(shaped.expression?.movementEffects),
          postureEffects: asArray(shaped.expression?.postureEffects),
          sensoryFocus: asArray(shaped.expression?.sensoryFocus),
          expressionPrinciples: asArray(shaped.expression?.expressionPrinciples),
          commonFailures: asArray(shaped.expression?.commonFailures),
          prohibitedMisreadings: asArray(shaped.expression?.prohibitedMisreadings),
          goldExampleRefs: uniqueStrings(shaped.expression?.goldExampleRefs),
          calibrationPairRefs: uniqueStrings(shaped.expression?.calibrationPairRefs),
          evidenceRefs: uniqueStrings(shaped.expression?.evidenceRefs)
        },
        claims: claims.map((claim) => ({
          ...
          evidenceRefs: uniqueStrings(claim.evidenceRefs),
          origin: claim.origin
        })),
        evidenceRefs,
        evidenceBindings: asArray(shaped.evidenceBindings),
        ragLayer: shaped.ragLayer,
        ragLayerLabel: shaped.ragLayer === 'category' ? '上位类别' : shaped.ragLayer === 'concept' ? '具体概念' : '',
        parentRagNodeIds: uniqueStrings(asArray(shaped.parentCardIds).map((id) => nodeId('rag', id))),
        role: 'card',
        linkedConceptIds: uniqueStrings(shaped.linkedConceptIds),
        linkedStoryIds: uniqueStrings(shaped.linkedStoryIds),
        linkedPlotIds: uniqueStrings(shaped.linkedPlotIds),
        linkedGameplayIds: uniqueStrings(shaped.linkedGameplayIds)
      }
    });

    if (!asArray(shaped.sourceRefs).length && !evidenceRefs.length) {
      nodes.get(ragNodeId).auditStatus = 'missing_source';
    }

    // 证据/来源不进大图节点：只挂在 RAG 详情「原文证据」页签（可展开段落 + 小说名列表）
    const evidenceItems = [];
    const sourceTitleSet = new Set();
    for (const evidenceId of evidenceRefs) {
      const excerpt = evidenceById.get(evidenceId);
      const binding = asArray(shaped.evidenceBindings).find((item) => item.evidenceId === evidenceId);
      const purpose = binding?.purpose || 'knowledge-evidence';
      const sourceId = excerpt?.sourceId || null;
      const catalog = sourceId ? sourceById.get(sourceId) : null;
      const sourceTitle =
        catalog?.title ||
        (excerpt?.location?.sourcePath
          ? String(excerpt.location.sourcePath).split(/[/\\]/).pop()?.replace(/\.(md|txt)$/i, '')
          : '') ||
        sourceId ||
        '';
      if (sourceTitle) sourceTitleSet.add(sourceTitle);
      const before = String(excerpt?.contextBeforePreview ?? '').trim();
      const body = String(excerpt?.excerptPreview ?? '').trim();
      const after = String(excerpt?.contextAfterPreview ?? '').trim();
      const paragraph = [before && `…${before}`, body, after && `${after}…`].filter(Boolean).join('\n');
      evidenceItems.push({
        evidenceId,
        purpose,
        reviewStatus: excerpt?.reviewStatus ?? 'pending',
        extractionOrigin: excerpt?.extractionOrigin ?? null,
        sourceId,
        sourceTitle,
        sourceMissing: Boolean(excerpt?.sourceMissing),
        location: excerpt?.location ?? null,
        rightsScope: excerpt?.rightsScope ?? { privateRag: true, publicExport: false },
        excerptPreview: body,
        contextBeforePreview: before,
        contextAfterPreview: after,
        paragraph: paragraph || body || evidenceId
      });
    }
    const ragNode = nodes.get(ragNodeId);
    ragNode.meta = {
      ...ragNode.meta,
      evidenceItems,
      sourceTitles: [...sourceTitleSet]
    };

    // Evidence remains collapsed in the ordinary overview, but is projected as
    // first-class nodes for the dedicated RAG evidence-path filter/focus view.
    // This keeps every displayed source relationship traceable to one excerpt.
    for (const item of evidenceItems) {
      const sourceNodeId = nodeId('source', item.sourceId || `missing:${item.evidenceId}`);
      const evidenceNodeId = nodeId('evidence', item.evidenceId);
      ensureNode({
        id: sourceNodeId,
        type: 'source',
        title: item.sourceTitle || item.sourceId || '未命名来源',
        summary: '',
        sourceIds: item.sourceId ? [item.sourceId] : [],
        origin: 'external-knowledge-source',
        auditStatus: item.sourceMissing ? 'missing_source' : item.reviewStatus === 'confirmed' ? 'confirmed' : 'pending_review',
        confidence: item.reviewStatus === 'confirmed' ? 1 : 0.55,
        visibility: { primaryTag: false, detailPanel: true, graph: true, searchable: true, ragRetrievable: false, overviewDefault: false },
        meta: { role: 'source', sourceId: item.sourceId || '', rightsScope: item.rightsScope }
      });
      ensureNode({
        id: evidenceNodeId,
        type: 'evidence',
        title: item.sourceTitle ? `${item.sourceTitle}｜片段` : `证据 ${item.evidenceId}`,
        summary: item.excerptPreview || '',
        description: item.paragraph || '',
        sourceIds: uniqueStrings([item.evidenceId, item.sourceId]),
        origin: 'external-knowledge-evidence',
        auditStatus: item.reviewStatus === 'confirmed' ? 'confirmed' : 'pending_review',
        confidence: item.reviewStatus === 'confirmed' ? 1 : 0.55,
        visibility: { primaryTag: false, detailPanel: true, graph: true, searchable: true, ragRetrievable: false, overviewDefault: false },
        meta: { role: 'evidence', evidenceId: item.evidenceId, purpose: item.purpose, location: item.location, rightsScope: item.rightsScope }
      });
      addEdge({ source: ragNodeId, target: evidenceNodeId, relationType: 'supported_by', sourceRef: 'evidenceBindings', confidence: item.reviewStatus === 'confirmed' ? 1 : 0.55, auditStatus: item.reviewStatus === 'confirmed' ? 'confirmed' : 'pending_confirm', autoGenerated: item.reviewStatus !== 'confirmed' });
      addEdge({ source: evidenceNodeId, target: sourceNodeId, relationType: 'excerpt_of', sourceRef: 'evidence.location', confidence: 1, auditStatus: item.sourceMissing ? 'missing_source' : item.reviewStatus === 'confirmed' ? 'confirmed' : 'pending_review', autoGenerated: true });
    }

    for (const claim of claims) {
      for (const evidenceId of uniqueStrings(claim.evidenceRefs)) {
        // 声明级核验结果仍记在 meta，便于详情展示；不投影证据节点
        const review = evidenceReviews.find(
          (item) => item.claimId === claim.id && item.evidenceId === evidenceId && item.reviewedBy === 'user'
        );
        if (!review) continue;
        const claimMeta = ragNode.meta.claims?.find((entry) => entry.id === claim.id);
        if (claimMeta) {
          claimMeta.evidenceReviews = uniqueStrings([
            ...(claimMeta.evidenceReviews || []),
...
function buildRagCardDescription(card) {
  // 概览只放短身份说明；完整定义进详情「知识」页签，避免与知识/证据重复刷屏。
  const layer = card.ragLayer === 'category' ? '上位类别' : card.ragLayer === 'concept' ? '具体概念' : '';
  const overall = card.overallStatus || '';
  const parts = [];
  if (layer) parts.push(layer);
  if (overall) parts.push(`总状态：${overall}`);
  if (card.cardType) parts.push(`类型：${card.cardType}`);
  return parts.join(' · ');
}
...
export function computeGraphStats(nodes, edges, extras = {}) {
  ...
  let ragMissingEvidenceCount = 0;
  let ragStubCount = 0;
  let pendingEvidenceCount = 0;
  let confirmedEvidenceCount = 0;
  ...
  for (const node of nodeList) {
    ...
    if (node.type === 'rag') {
      if (node.meta?.contentStatus === 'stub') ragStubCount += 1;
      const evidenceItems = asArray(node.meta?.evidenceItems);
      evidenceNodeCount += evidenceItems.length;
      sourceNodeCount += asArray(node.meta?.sourceTitles).length;
      for (const item of evidenceItems) {
        if (item.reviewStatus === 'confirmed') confirmedEvidenceCount += 1;
        else if (item.reviewStatus !== 'rejected') pendingEvidenceCount += 1;
      }
      if (
        node.meta?.evidenceStatus === 'missing' ||
        !(node.meta?.evidenceRefs ?? []).length
      ) {
        ragMissingEvidenceCount += 1;
      }
    }
  }
