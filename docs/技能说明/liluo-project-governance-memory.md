...
分类持久性 → 定位权威源 → 查影响映射 → 主 Codex 修改权威源和必要消费者 → 按需记录 ADR/CDR → `liluo-project-documentation-sync` 做机械同步 → 治理脚本验证 → 项目索引增量更新。
...
不自动删除、压缩、重编号、保存聊天记录或把未确认决定标为 accepted。当前分类器是确定性提示器，复杂语境由主 Codex 判断。
