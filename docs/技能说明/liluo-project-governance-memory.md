...
分类持久性 → 定位权威源 → 查影响映射 → 主 Codex 修改权威源和必要消费者 → 按需记录 ADR/CDR → `liluo-project-documentation-sync` 做必要的机械同步 → 一项目标验证与至多一项相关综合验证 → 被索引源变化时末尾增量更新一次。
...
目标验证被综合 profile 覆盖时不重复执行，相关文件未再次变化时不重跑；未修改领域不验证，已知无关旧问题的全量审计不作为普通门禁。不自动删除、压缩、重编号、保存聊天记录或把未确认决定标为 accepted。当前分类器是确定性提示器，复杂语境由主 Codex 判断。
