...
- 所有 Grok prompt 都先走项目级图像治理 preflight；规则 authority 统一来自 [项目图像生成判层与治理系统](../系统说明/项目图像生成判层与治理系统.md)。
- 单次 Grok prompt 只装配命中的 authority / prompt / QA 层，不把全部长期规则全文塞进 dry-run 或 live。
...
- 如果这轮任务还要同步图像治理文档，先改 authority 与直接消费者，再决定是否在末尾执行生成型写回。
