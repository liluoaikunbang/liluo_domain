...
完成度与标题形态不决定层级。同一旧条目允许拆成 RAG + 情节。用户是最终裁决者；AI 只审计与建议。未确认不得创建正式 RAG、删除/改写情节类型、改故事引用或伪装图谱迁移完成。确认后的调整必须回写真实主数据并保留来源与映射。不得恢复退役 Tag 作为中间层。
...
跨层级调整以对话中的逐项确认完成，不维护独立审核队列或运行界面。若未来引入非紧缚通用 RAG 域，须另立审批，不得静默改 domain。批量确认仅在用户显式选择批次且排除 uncertain/冲突项时允许。
