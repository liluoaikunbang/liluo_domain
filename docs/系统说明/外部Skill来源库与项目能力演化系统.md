...
`external-knowledge/tools/` 只存放为执行某项工作流而下载的外部工具副本，统一受 Git 忽略，绝不作为项目源码、子模块或 GitHub 上传内容。新电脑首次运行依赖此类工具的工作流时，Codex 必须先检查所需目录是否存在；若缺失，应明确告知用户“需要由 Codex 下载对应工具”，说明上游来源与用途，并在用户确认后自行下载到该目录。不得把缺失伪装成工作流完成，也不得未经确认自动联网下载。

工具副本是可重建的本机依赖：其来源、受控路径、版本和更新追踪仍按本系统登记；下载完成后仍不自动升级，后续更新继续遵守只读跟踪与用户批准边界。
