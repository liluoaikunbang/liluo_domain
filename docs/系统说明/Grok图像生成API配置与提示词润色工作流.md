...
统一治理规则见 [项目图像生成判层与治理系统](./项目图像生成判层与治理系统.md)。Grok 不是例外链路；它只是项目图像生成体系中的一个可选模型入口。

这套工作流是项目的外部图像生成补充链路，不是海报升级、角色基线校准、README/Pages 视觉批次或其他既有 `Image 2` 资产的默认入口。凡是项目已经明确登记为 `Image 2` 的视觉任务，默认优先使用内置 `Image 2`；只有用户明确要求 Grok，或需要检查 / 调试 Grok 专属链路时，才使用本文流程。
...
如果是璃落相关图像，进入 live 前至少要覆盖年龄与成年边界、发色、瞳色和非幼态边界；娇小体型、自然曲线与柔和鹅蛋脸属于应继续补齐的扩展锚点。
...
现在 live 前还会运行一次项目图像治理 preflight：

- 如果只是探索型 dry-run，会返回待补齐的身份锚点和治理提醒。
- 如果 prompt 明确在画璃落，却缺少核心身份锚点，live 会直接阻断。
- 这套 preflight 和站点 `Image 2` 批次使用同一份项目 authority，不再各自维护一套提示词口径。
...
- 如果当前任务属于官网海报、世界图、角色基线、README/Pages 视觉批次等既有 `Image 2` 队列，不要因为 Grok 已配置就越过默认优先级。
