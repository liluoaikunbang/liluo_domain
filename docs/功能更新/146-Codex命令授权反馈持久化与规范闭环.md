...
- 更新日期：2026-08-01（把稳定 `EPERM` 的报告命令与 Windows 大块中文补丁失配纳入低绕圈执行规则）
- 当前状态：已完成自动化、execpolicy 与当前客户端精确前缀沙箱外回归
- 当前摘要：建立四级授权与精确规则，并以非叠加验证预算、固定最小 profile、稳定 `EPERM` 命令直走精确路线和局部补丁策略减少重复检查、无信息 RED 与沙箱失败重试。
...
2026-08-01 根据资产治理与治理文档编辑中的真实回放，进一步把“可预判的失败路径”固化进权威规则：对已经稳定证明会因写回 `docs/assets/registry/*.json` 等受管报告文件而在沙箱内返回 `EPERM` 的正式命令，首轮直接走精确沙箱外路线，不再先制造一次失败再重试；对包含长中文段落、Markdown 表格、反斜杠路径与混合引号的大块文本编辑，首次 `apply_patch` 失配后立即切换为局部精确 hunk，不再重复提交同一份宽范围替换。这样保留平台审批边界，但尽量消除无信息重试和额外 token 消耗。
