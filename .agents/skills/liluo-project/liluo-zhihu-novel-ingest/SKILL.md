...
- `external-knowledge/tools/zhihu-download/` is a Git-ignored, local-only tool clone, never a submodule or upload target. Before downloading, check that it exists. On a new computer, if it is absent, tell the user that Codex needs to download the Zhihu downloader from `https://github.com/chenluda/zhihu-download.git` for this workflow; clone it into the tool path only after the user confirms. Do not silently download it or claim the workflow completed while it is missing.
