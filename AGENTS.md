...
项目工作流位于 `.agents/skills/liluo-project/`。任务符合某个 Skill 的描述时，必须先读取并遵循该 Skill；完整入口见 [用户命令目录](docs/用户命令目录.md)，不要把执行细则复制回本文。
...
生成或修改非技术、面向阅读体验的文字时，默认应用 `liluo-natural-expression` 的 light；技术合同、路径、代码与测试日志不文学化。正式项目中所有登场、受描写或参与情节的角色均为成年人。
...
命令的长期允许、每次询问或禁止，使用 `liluo-command-approval-governance`；单次允许不持久化，不改写用户级规则。普通编辑按用户任务授权执行，项目 Skill、Agent、规则和配置均直接使用 `apply_patch`，不得再按文件或目录逐项询问；生成物只走正式生成器。网络、依赖、发布、凭据、`.git` 写入与正式删除仍遵守平台边界。用户明确要求上传 GitHub 时，完成本地审查、暂存和提交后直接执行一次目标 `git push`，不得默认先用 `git ls-remote`、fetch 或其他联网命令预检远端。验证选择 `npm run project:routine -- <最小适用 profile>`，按变化面去重，不以 `all` 代替范围判断；完整规则见 [Codex 命令授权治理系统](docs/系统说明/Codex命令授权治理系统.md)。生命周期 Hook 仅作输入级常数时间判断，禁止 Stop Hook、仓库扫描和质量门禁，详见 [Codex Hooks 与自动质量门禁系统](docs/系统说明/Codex-Hooks与自动质量门禁系统.md)。
...
项目组人格与子智能体边界遵循 `liluo-creative-team-presence` 及 `docs/设计记忆/项目组灵魂/`：人格不改变权限、事实或用户审批，只有真实参与的 Agent 才能形成正式观点。小型明确任务不委派；普通复杂任务最多并行三个，子智能体默认只读，主 Codex 统一写入，最大嵌套深度为 1。
...
- 项目 Skill、用户入口、系统行为或数据契约的实质变化按 `liluo-project-documentation-sync` 同步。新增 `src/assets/game` 图片时运行 `liluo-asset-registry-audit`。
