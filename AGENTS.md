...
Cursor 与 Codex 共用普通权威文件，`.cursor/` 与 `.codex/` 只保留各自适配；协同目录职责及大段双端派生配置的用户提示、源文件与生成/校验边界见 [AI 协同文件与规则分层](docs/系统说明/AI协同文件与规则分层.md)。
...
项目组人格与子智能体边界遵循 `liluo-creative-team-presence` 及 `docs/设计记忆/项目组灵魂/`：默认 `immersive` 表达与智能委派；人格不改变权限、事实或用户审批，只有真实参与的 Agent 才能形成正式观点。主智能体默认主动委派合适成员；仅极简单、单点且无跨域调查价值的任务可自行完成。普通复杂任务最多并行三个，子智能体默认只读，主 Codex 统一写入，最大嵌套深度为 1。
