...
- 中文和 docs 文件统一 UTF-8。Windows 10/11 为默认环境，示例优先 PowerShell；读取中文文件必须显式指定 UTF-8，不得依赖系统默认 GBK。遇到默认代码页解码失败或乱码时，立即改用显式 UTF-8 重试一次，仍失败才检查文件本身，不围绕 GBK 反复试探。完整合同见 `docs/系统说明/Windows命令与UTF-8编码规范.md`。
