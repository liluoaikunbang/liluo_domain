# 璃落宇宙：AI 原生叙事游戏生产体系核心约束

这是一个长期生长的 AI 原生叙事游戏生产体系，`璃落宇宙` 是它当前最重要、最长期维护的旗舰实例。项目要把同一宇宙中的故事、角色、状态和氛围，稳定生产成多种可玩、可读、可扩展的体验形态。

像素风冒险 RPG 仍是当前旗舰体验：地图移动、探索、事件、剧情推进、角色与场景氛围是核心；互动小说、副本、文本桥段、资料页、收集、家具和功能页面都只能作为同一叙事生产体系的输出与补强，不能反客为主，也不能把项目做成纯 GalGame、纯文本工具链或功能页面集合。
...
- **任何将项目内容写入故事、情节、地图事件、对话、玩法或局部设定的任务**，在正式写回前必须使用 `liluo-rag-candidate-proposal`：自动全面拆分本次增量，只生成可追溯的 RAG 候选与来源待办；用户确认前不得新建/关联 RAG、确认知识或扫描外部文章。用户要求补扫既有内容时复用该 Skill；用户明确要求扫描来源时才使用 `liluo-rag-source-association`，默认 dry-run 且逐条确认。所有 RAG 卡均固定拥有知识与表达双分支，可为空但不得缺结构。
- 从情节库抽取未用或指定情节、对照大纲建议主线/支线落点并在批准后访谈时，使用 `liluo-plot-placement-interview`；用户说“随机提问”或“继续随机提问”时，默认是大纲节点优先的随机访谈，使用 `random-story-outline-interview`。
...
- 用开放权重模型写正式小说段落、双模型对照、写作 API 检查、个人旧作/黄金正文/修改对照归档或模型校准时，使用 `liluo-formal-prose-pipeline`；候选只进工作区，批准后由现有故事 Skill 写入正式内容。所有 RAG 卡固定知识+表达双分支，每个分支均须经用户确认后才可写入；不得恢复独立通用 Style-RAG、自动文风检索或 AI 自动生成的写法条目。大纲「关联图谱」的「RAG 网络」为自由画布，可浏览概念、证据片段与来源的已确认关系；全图节点默认只显示名称，不改写正史。详见 [开放权重双模型写作管线](docs/系统说明/开放权重双模型写作管线.md)、[紧缚专业RAG双分支系统](docs/系统说明/紧缚专业RAG双分支系统.md)、[知识检索抽查校准系统](docs/系统说明/知识检索抽查校准系统.md) 与 [大纲关联图谱系统](docs/系统说明/大纲关联图谱系统.md)。
...
- 用户说“情节”“桥段”或“情节库”时，直接写入 `src/game/data/plot_outline/catalog.json`；说“大纲”“主线”或“节点”时才进入故事大纲来源。两库仅在用户明确要求关联时同时处理；`主线任务`只作容器，绝不可按普通故事块套模板。用户提供的情节若已能支撑独立冲突、玩家体验或前后状态衔接，应优先提议建立普通故事节点并向用户确认，而不是继续堆入主线备忘；详见 [故事大纲条目模板](docs/系统说明/故事大纲条目模板.md)。新元素若可能跨层级（概念 / 一次性事件 / 正式故事），须先按 [故事情节RAG边界治理系统](docs/系统说明/故事情节RAG边界治理系统.md) 给出建议并等待用户确认；未经确认不得批量改写情节库或迁入 RAG。
- 每次更新主线任务之下的节点（包括局部设定补丁）前，必须按 [故事大纲条目模板](docs/系统说明/故事大纲条目模板.md) 核对该主线及所属世界类别的趋势锚点；新增内容须明确保持或推进至少一项既定趋势。趋势来源缺失、含义不明或出现冲突时，先向用户确认，不得自行补全或写入。
...
生成或修改非技术、面向阅读体验的文字时（含试笔、测试写作、用户点名的绑法/场景草稿），默认先读并应用 `liluo-natural-expression` 的 light，再动笔；技术合同、路径、代码与测试日志不文学化。**RAG 知识/表达分支、抽查协商与改稿说明**同属阅读向文字：撰写或改写前须走同一 light；不得用套话、空泛层级术语代替可检验的具体说法。命名绑法或紧缚过程是看点时，交付前必须自检「名称 ↔ 识别姿态」是否一致；正式写入故事/对话文件且涉及命名绑法时，可再委派 `liluo_content_auditor`（砚秋）做只读术语与一致性抽查。正式项目中所有登场、受描写或参与情节的角色均为成年人。
...
最重要的判断标准：项目应越来越像一个能够把同一叙事稳定生产为多种体验形态的 AI 原生叙事游戏生产体系，并始终保有像素风冒险 RPG 作为旗舰世界体验，而不是退化成一堆功能页面、脱离世界的文本管线或只剩工具外壳的系统。
